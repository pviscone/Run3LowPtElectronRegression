...
def plot_distributions(
    df, features=None, weight=None, log=False, savefolder="plots/distributions"
):
    if features is None:
        keys = df.columns
    else:
    ...
    for eta_min, eta_max in zip(eta_bins[:-1], eta_bins[1:]):
        ...
        for genp_min, genp_max in zip(genp_bins[:-1], genp_bins[1:]):
            ...
            for idx, (label, pratio) in enumerate(pratio_dict.items()):
                ...
                var = np.sum(
                    ((genp_eta[mask_genp] * np.abs(pratio_masked - 1)) ** 2)
                    / (len(genp_eta[mask_genp]) - 1)
                )
            ...
            if plot_distributions:
                ...
                for idx, (label, pratio) in enumerate(pratio_dict.items()):
                    ...
                    ax.set_title(
                        f"$| \eta |$: [{eta_min},{eta_max}], GenP: [{genp_min},{genp_max}]"
                    )
                    ...
                    ax.axvline(
                        ...
                        color=colors[idx],
                        linestyle="--",
                        label=f"Median {label}: {median:.2f}",
                        alpha=0.7,
                    )
                    ax.fill_betweenx(
                        y=[0, ax.get_ylim()[1]],
                        ...
                        color=colors[idx],
                        alpha=0.2,
                        label=f"16-84% {label}: [{perc16:.2f}, {perc84:.2f}]",
                    )
                ...
                fig.savefig(
                    f"{savefolder}/distributions/pratio_eta_{str(eta_min).replace('.', '')}_{str(eta_max).replace('.', '')}_genp_{str(genp_min).replace('.', '')}_{str(genp_max).replace('.', '')}.pdf"
                )
                plt.close(fig)
        ...
        for idx, label in enumerate(pratio_dict.keys()):
            ...
            ax.plot(
                centers[label] + idx * diff * 0.3 - diff / 4,
                medians[label],
                ".",
                marker=markers[idx],
                label=f"$\\bf{{{median_label}}}$\nMedian",
                color=colors[idx],
        ...
        fig.savefig(
            f"{savefolder}/response/aresponse_eta_{str(eta_min).replace('.', '')}_{str(eta_max).replace('.', '')}.pdf"
        )
        fig.savefig(
            f"{savefolder}/response/aresponse_eta_{str(eta_min).replace('.', '')}_{str(eta_max).replace('.', '')}.png"
        )
        plt.close(fig)
        ...
        fig.savefig(
            f"{savefolder}/resolution/resolution_eta_{str(eta_min).replace('.', '')}_{str(eta_max).replace('.', '')}.pdf"
        )
        fig.savefig(
            f"{savefolder}/resolution/resolution_eta_{str(eta_min).replace('.', '')}_{str(eta_max).replace('.', '')}.png"
        )
        plt.close(fig)
...
def plot_calibration_curve(
    ...
    eta_col = "LPEle_eta"
    savefolder = f"{savefolder}/calibration"

    if sigma_bins is None:
        sigma_bins = np.arange(0, 2, 0.1)
    ...
    def _plot_for_mask(y, mu, sigma, suffix, metric=metric):

        centers = np.array([])
        meds = np.array([])

        for sigma_low, sigma_high in zip(sigma_bins[:-1], sigma_bins[1:]):
            bin_mask = (sigma >= sigma_low) & (sigma < sigma_high)
            if bin_mask.sum() == 0:
                continue
            y_masked = y[bin_mask]
        ...
        ax.plot(centers, meds, "o-", label=f"Median $\sigma$ ({metric})", linewidth=2, zorder = 10)

        ax.set_xlabel("Predicted uncertainty")
        ax.set_ylabel("Empirical uncertainty")
        ax.set_title(f"{suffix.replace('_', ' ').replace('eta', '|$\eta|').replace('genp', '$p^{Gen}$')}")
        ax.legend()
        fig.savefig(
            f"{savefolder}/calibration_curve_{suffix.replace(' ', '_').replace('[', '').replace(']', '').replace(',', '_')}.pdf"
        )
        fig.savefig(
            f"{savefolder}/calibration_curve_{suffix.replace(' ', '_').replace('[', '').replace(']', '').replace(',', '_')}.png"
        )
        plt.close(fig)
    ...
    if not genp_provided and not eta_provided:
        _plot_for_mask(y, mu, sigma, "global")
        return
    ...
    if genp_provided and not eta_provided:
        ...
        for lo, hi in zip(genp_bins[:-1], genp_bins[1:]):
            ...
            _plot_for_mask(y[mask], mu[mask], sigma[mask], f"genp_{lo:.2f}_{hi:.2f}")
        return
    ...
    if eta_provided and not genp_provided:
        ...
        for lo, hi in zip(eta_bins[:-1], eta_bins[1:]):
            mask = (eta >= lo) & (eta < hi)
            _plot_for_mask(y[mask], mu[mask], sigma[mask], f"eta_{lo:.2f}_{hi:.2f}")
        return
