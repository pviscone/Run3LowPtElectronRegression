...
def response_resolution(df,
    ...
    for eta_min, eta_max in zip(eta_bins[:-1], eta_bins[1:]):
        ...
        medians={key:np.array([]) for key in ptratio_dict.keys()}
        perc16s={key:np.array([]) for key in ptratio_dict.keys()}
        perc84s={key:np.array([]) for key in ptratio_dict.keys()}
        perc5s={key:np.array([]) for key in ptratio_dict.keys()}
        perc95s={key:np.array([]) for key in ptratio_dict.keys()}
        residuals = {key:np.array([]) for key in ptratio_dict.keys()}
        variances = {key:np.array([]) for key in ptratio_dict.keys()}
        n = {key:np.array([]) for key in ptratio_dict.keys()}
        for genpt_min, genpt_max in zip(genpt_bins[:-1], genpt_bins[1:]):
            for idx, (label, ptratio) in enumerate(ptratio_dict.items()):
                ...
                centers[label] = np.append(centers[label],((genpt_min + genpt_max) / 2))
                ...
                n[label] = np.append(n[label],(len(ptratio_masked)))
            ...
            if plot_distributions:
                ...
                for idx, (label, ptratio) in enumerate(ptratio_dict.items()):
                    ...
                    ax.set_xlabel("Reco $p_{T}$ / $p_{T}^{\\text{Gen}}$")
                    ax.set_ylabel("Density")
                    ax.legend(fontsize=15)
                    h = hist.Hist(hist.axis.Regular(29, 0.3, 1.7, name="ptratio", label="Reco $p_{T}$ / $p_{T}^{\\text{Gen}}$"))
                ...
                #fig.savefig(f"{savefolder}/distributions/ptratio_eta_{str(eta_min).replace('.','')}_{str(eta_max).replace('.','')}_genpt_{str(genpt_min).replace('.','')}_{str(genpt_max).replace('.','')}.png")
                fig.savefig(f"{savefolder}/distributions/ptratio_eta_{str(eta_min).replace('.','')}_{str(eta_max).replace('.','')}_genpt_{str(genpt_min).replace('.','')}_{str(genpt_max).replace('.','')}.pdf")
                plt.close(fig)


        legend_order = []
        counter=0
        for i in range(3*len(ptratio_dict)):
            if i%3==0:
        ...
        ax.set_ylabel("$p_{T}^{\\text{Reco}}$ / $p_{T}^{\\text{Gen}}$")
        ...
        ax.set_xlabel("$p_{T}^{\\text{Gen}}$ [GeV]")
        ax.set_ylabel("$\sigma_{eff}$($p_{T}^{\\text{Reco}}$ / $p_{T}^{\\text{Gen}}$)")
        for idx, label in enumerate(ptratio_dict.keys()):
