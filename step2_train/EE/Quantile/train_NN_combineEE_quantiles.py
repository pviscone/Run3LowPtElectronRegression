...
import os
import sys
from utils.features import combined_features, common_features, to_log, caloEE_features, tk_features
...
caloFeatures = common_features + caloEE_features
tkFeatures = common_features + tk_features

caloModel = xgb.Booster()
caloModel.load_model("BDT_caloEE_quantile.json")
tkModel = xgb.Booster()
tkModel.load_model("BDT_tkEE_quantile.json")
...
for i,q in enumerate([0.05, 0.16, 0.5, 0.84, 0.95]):
    ...
    features_ee.append(f"calo_quantile_{q}")
    features_ee.append(f"tk_quantile_{q}")

features_ee += common_features + combined_features
...
def callback(model, savefolder, nepochs):
    ...
    out = model.f(df_test[features_ee].to_numpy())
