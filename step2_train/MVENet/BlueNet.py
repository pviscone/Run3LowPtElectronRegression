...
class BlueNet(nn.Module):
    ...
    def pred(self, X_test, pECorr):
        ...
        pECorr = torch.tensor(pECorr, dtype=torch.float32).to(self._device)

        self.eval()
        with torch.no_grad():
            out = self(X_test)
            p = out[:, 0]*pECorr[:, 0] + (1 - out[:, 0])*pECorr[:, 1]
            return p.cpu().numpy()
    ...
    def init_mu(self):
        ...
        with torch.no_grad():
            self.mean1_branch[-1].bias.data = torch.ones_like(
    ...
    def train_model(
        ...
        if (
            ...
            and weight_decay == self.fit_kwargs[-1].get("weight_decay", 0)
        ...
        pECorr = torch.tensor(pECorr, dtype=torch.float32).to(self._device)
        if validation:
            pECorr_val = torch.tensor(pECorr_val, dtype=torch.float32).to(self._device)
        ...
        for epoch in range(n_epochs):
            ...
            for start in range(0, X_train.shape[0], batch_size):
                ...
                pECorr_b = pECorr[batch_idx]
                optimizer.zero_grad()
                out = self(xb)

                estimate = (out[:, 0]*pECorr_b[:, 0] + (1 - out[:, 0])*pECorr_b[:, 1]).unsqueeze(-1)/yb
                out = torch.cat([estimate, out[:, 1:]], dim=-1)

                ones = torch.ones_like(yb)

                loss_samples = self.loss.loss(ones, out)
                if wb is not None:
            ...
            if validation:
                ...
                with torch.no_grad():
                    out_val = self(X_val)
                    estimate_val = (out_val[:, 0]*pECorr_val[:, 0] + (1 - out_val[:, 0])*pECorr_val[:, 1]).unsqueeze(-1)/Y_val
                    out_val = torch.cat([estimate_val, out_val[:, 1:]], dim=-1)

                    ones_val = torch.ones_like(Y_val)

                    loss_val = self.loss.loss(ones_val, out_val).mean()
