...
class DoubleMVENet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.model_kwargs = {
            ...
            "n_hidden_mean2": n_hidden_mean2,
            "n_hidden_var2": n_hidden_var2,
            "n_hidden_rho": n_hidden_rho,
            "dropout_input": dropout_input,
            "dropout_common": dropout_common,
            "dropout_mean1": dropout_mean1,
            "dropout_var1": dropout_var1,
            "dropout_mean2": dropout_mean2,
            "dropout_var2": dropout_var2,
            "dropout_rho": dropout_rho,
            "loss": loss,
        ...
        # Mean branch
        mean2_layers = []
        mean2_in = in_features
        for n in n_hidden_mean2:
            if dropout_mean2 > 0:
                mean2_layers.append(nn.Dropout(dropout_mean2))
            mean2_layers.append(nn.Linear(mean2_in, n))
            mean2_layers.append(nn.ELU())
            mean2_in = n
        mean2_layers.append(nn.Linear(mean2_in, 1))
        self.mean2_branch = nn.Sequential(*mean2_layers)

        # Variance branch
        var2_layers = []
        var2_in = in_features
        for n in n_hidden_var2:
            if dropout_var2 > 0:
                var2_layers.append(nn.Dropout(dropout_var2))
            var2_layers.append(nn.Linear(var2_in, n))
            var2_layers.append(nn.ELU())
            var2_in = n
        var2_layers.append(nn.Linear(var2_in, 1))
        self.var2_branch = nn.Sequential(*var2_layers)

        rho_layers = []
        rho_in = in_features
        for n in n_hidden_rho:
            if dropout_rho > 0:
                rho_layers.append(nn.Dropout(dropout_rho))
            rho_layers.append(nn.Linear(rho_in, n))
            rho_layers.append(nn.ELU())
            rho_in = n
        rho_layers.append(nn.Linear(rho_in, 1))
        rho_layers.append(nn.Tanh())
        self.rho_branch = nn.Sequential(*rho_layers)
    ...
    def rho(self, X_test):
        ...
        with torch.no_grad():
            out = self(X_test)
            rho = self.loss.rho(out.cpu().numpy())
            return rho
    ...
    def BLUE(self, X_test, x, y):
        ...
        self.eval()
        with torch.no_grad():
            mu, sigma, rho = self.f(X_test)
            mu_x, mu_y = mu[:,0]*x, mu[:,1]*y
            sigma_x, sigma_y = sigma[:,0]*x, sigma[:,1]*y
            cov_xy = rho * sigma_x * sigma_y

            denom = sigma_x**2 + sigma_y**2 - 2*cov_xy
            w0 = (sigma_y**2 - cov_xy) / denom
            w1 = 1 - w0

            mu_comb = w0 * mu_x + w1 * mu_y

            sigma_comb2 = w0**2 * sigma_x**2 + w1**2 * sigma_y**2 + 2*w0*w1*cov_xy
            sigma_comb = np.sqrt(sigma_comb2)
    ...
    def init_variance(self, X_train, Y_train):
        ...
        with torch.no_grad():
            ...
            logmse2 = 0.5*np.log(np.mean(np.square(mu2 - y2_true)))
            self.var1_branch[-1].bias.data = torch.tensor(
                [logmse1],
                dtype=torch.float32,
                device=self.var1_branch[-1].bias.device,
            )
            self.var2_branch[-1].bias.data = torch.tensor(
                [logmse2],
                dtype=torch.float32,
    ...
    def get_loss(self):
        train_loss = [item for fit_kwarg in self.fit_kwargs for item in fit_kwarg["train_loss"]]
        val_loss = [item for fit_kwarg in self.fit_kwargs for item in fit_kwarg.get("val_loss", [])]
    ...
    def train_model(
        ...
        for epoch in range(n_epochs):
            ...
            for start in range(0, X_train.shape[0], batch_size):
                ...
                l2_mean2 = reg_mean2 * nn_utils.get_l2_reg(self.mean2_branch)
                l2_var2 = reg_var2 * nn_utils.get_l2_reg(self.var2_branch)
                l2_rho = reg_rho * nn_utils.get_l2_reg(self.rho_branch)
                full_loss = loss + l2_common + l2_mean1 + l2_var1 + l2_mean2 + l2_var2 + l2_rho
                full_loss.backward()
            ...
            if verbose:
                if validation:
                    ...
                    print(f"Epoch {self.model_kwargs["n_epochs"]}/{initial_epoch+n_epochs}; Train Loss: {self.train_loss[-1]:.4f}")

            if checkpoint is not None and (epoch + 1) % checkpoint == 0:
                os.makedirs(savefolder, exist_ok=True)
                self.save(os.path.join(savefolder, f"epoch_{self.model_kwargs["n_epochs"]}"))
                print(f"Model checkpoint saved at epoch {self.model_kwargs["n_epochs"]} to {savefolder}")
            if callback_every is not None and (epoch + 1) % callback_every == 0 and callback is not None:
                callback(self, os.path.join(savefolder, f"epoch_{self.model_kwargs["n_epochs"]}"), self.model_kwargs["n_epochs"])
